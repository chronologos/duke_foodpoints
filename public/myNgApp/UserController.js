  ...
  $http.get('/api/user')
    .success(function(data, status, headers, config) {
      if (status === 200) {
          ...
          DEBUG && console.log("angular got a user, " + JSON.stringify($scope.user));

          DEBUG && console.log("Length of balances is " + $scope.user.balances.length);
          var trans = getTrans($scope.user.balances);
          //$scope.user.trans = trans;
          DEBUG && console.log("Number of different transactions detected for client: " + trans.length);
          ...
          DEBUG && console.log("Printing result of favList");
          for (var i = 0; i < favList.length; i ++) {
            DEBUG && console.log(favList[i][0] + " : " + favList[i][1]);
          }
          $scope.user.allFavs = favList;
          DEBUG && console.log("allFavs is " + $scope.user.allFavs);


          DEBUG && console.log("User's favorite item costs " + $scope.user.fav + " and it was bought " + $scope.user.numFav + " times");
      }
...
function getTrans(bals) {
  DEBUG && console.log("Type of input to getTrans method: " + typeof(bals));
  var arr = [];
...
function getFreqs(trans) {
  DEBUG && console.log("Type of input to getFreqs method: " + typeof(trans));
  var freqs = {};
...
function getFav(trans) {
  ...
  ***/
  ...
  DEBUG && console.log("Fav is " + fav + " which was bought " + maxCount + " times");
  return [fav, maxCount];
}
  ...
  var done = 0;
    ...
    firstKeys.forEach(function(y) {
      ...
      //  result[y] = x;
        ...
        DEBUG && console.log("Added key " + y + " with value " + x);
        done ++;
      }
    });
  });
